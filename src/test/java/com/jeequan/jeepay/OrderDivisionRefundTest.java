...
import com.jeequan.jeepay.model.OrderDivisionRefundQueryReqModel;
import com.jeequan.jeepay.model.OrderDivisionRefundReqModel;
import com.jeequan.jeepay.request.OrderDivisionRefundQueryRequest;
import com.jeequan.jeepay.request.OrderDivisionRefundRequest;
import com.jeequan.jeepay.response.OrderDivisionRefundQueryResponse;
import com.jeequan.jeepay.response.OrderDivisionRefundResponse;
...
/** 订单分账回退测试 **/
class OrderDivisionRefundTest {

    final static Logger _log = LoggerFactory.getLogger(OrderDivisionRefundTest.class);
    ...
    public void testPayOrderDivisionRefund() {
        ...
        OrderDivisionRefundRequest request = new OrderDivisionRefundRequest();
        OrderDivisionRefundReqModel model = new OrderDivisionRefundReqModel();
        request.setBizModel(model);
        model.setMchNo(Jeepay.mchNo);                       // 商户号
        model.setAppId(jeepayClient.getAppId());            // 应用ID
//        model.setMchDivisionNo("1741080214799");            // 商户分账单号
        model.setBatchOrderId("D1991775726426714114");            // 商户分账单号
        model.setMchRefundNo(System.currentTimeMillis() + "");
        model.setReceiverId(800103L);
        model.setRefundAmount(1L);
        model.setRemark("测试abc");

        try {
            OrderDivisionRefundResponse response = jeepayClient.execute(request);
    ...
    public void testBalanceDivisionRefundQuery() {
        // 分账接口文档：https://docs.jeequan.com/docs/jeepay/division_api
        JeepayClient jeepayClient = JeepayClient.getInstance(Jeepay.appId, Jeepay.apiKey, Jeepay.getApiBase());
        OrderDivisionRefundQueryRequest request = new OrderDivisionRefundQueryRequest();
        OrderDivisionRefundQueryReqModel model = new OrderDivisionRefundQueryReqModel();
        request.setBizModel(model);
        model.setMchNo(Jeepay.mchNo);                       // 商户号
        model.setAppId(jeepayClient.getAppId());            // 应用ID
        // model.setPayOrderId("P1792369673117708289");
        model.setDivisionRefundId("R1993511352851288066");
//        model.setReceiverId(800001L);


        try {
            OrderDivisionRefundQueryResponse response = jeepayClient.execute(request);
            _log.info("验签结果：{}", response.checkSign(Jeepay.apiKey));
            if(response.isSuccess(Jeepay.apiKey)) {
